...
#endif
/* Time data type defintion */
#ifdef MINIUPNPC_SET_SOCKET_TIMEOUT
#include <sys/time.h>
#endif

/* miniupnpc self defined header */
#include "miniupnpc.h"
#include "miniwget.h"
#include "minisoap.h"
#include "minixml.h"
...
{
	// example define char array!
	static const char contlenstr[] = "content-length";
	// const pointer： which can not change reference variable value?
	// pointer const： can not pointer to another variable 
                      ...
                      char * buffer, int * bufsize)
{
	...
	} // prepare content of command

	if (!parseURL(url, hostname, &port, &path)) 
		return -1;
	
	if (s<0) {	// no socket exits, then create a socket
        ...
        n = connect(s, (struct sockaddr *)&dest, sizeof(dest));
                              ...
                              const char * minissdpdsock, int sameport)
{
    ...
    /* reception */
    memset(&sockudp_r, 0, sizeof(sockudp_r));
    sockudp_r.sin_family = AF_INET;
	if(sameport)
    	sockudp_r.sin_port = htons(PORT);
	// bind local any ip allocated by kernel 
    sockudp_r.sin_addr.s_addr = INADDR_ANY;

    /* emission */
    memset(&sockudp_w, 0, sizeof(sockudp_w));
	...
	/* bind sock to a local address for server to get address */
    if (bind(udpsock, (struct sockaddr *)&sockudp_r, sizeof(struct sockaddr_in)) != 0) {
